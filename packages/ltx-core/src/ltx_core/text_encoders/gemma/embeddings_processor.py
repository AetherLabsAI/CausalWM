...
class EmbeddingsProcessor(nn.Module):
    ...
    def create_embeddings(
        ...
        # Padmask mode: connector kept padded positions intact; pass the additive
        # mask straight through so DiT cross-attn can softmax-zero them. The DiT
        # `_prepare_attention_mask` accepts either binary or additive masks
        # (see ``_prepare_attention_mask``). Return ``video_mask`` (connector output)
        # rather than ``mask_for_connector`` so any future mask normalization
        # inside the connector flows out — the two are identical today, but
        # going through the connector's return value preserves the contract.
        if getattr(self.video_connector, "apply_padding_mask", False):
            return video_encoded, audio_encoded, video_mask
