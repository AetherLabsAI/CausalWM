...
def module_ops_from_gemma_root(gemma_root: str) -> tuple[ModuleOps, ...]:
    ...
    def load_tokenizer(module: GemmaTextEncoder) -> GemmaTextEncoder:
        # Text context length is 128 tokens (the base model uses 1024). This cuts
        # the connector's self-attention cost and the DiT cross-attention K/V
        # length. Longer prompts are truncated.
        module.tokenizer = LTXVGemmaTokenizer(tokenizer_root, 128)
        return module
