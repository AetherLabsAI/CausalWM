...
class Embeddings1DConnector(torch.nn.Module):
    ...
    def __init__(
        ...
        self.apply_padding_mask = apply_padding_mask
    ...
    def forward(
        ...
        """
        # Padmask mode: keep additive mask flowing through self-attn so padded
        # positions softmax to ~0; skip register replacement. Register parameter
        # is still allocated (loaded from ckpt) but unused in forward.
        if self.num_learnable_registers and not self.apply_padding_mask:
...
class Embeddings1DConnectorConfigurator(ModelConfigurator[Embeddings1DConnector]):
    ...
    def from_config(cls: type[Embeddings1DConnector], config: dict) -> Embeddings1DConnector:
        ...
        connector = Embeddings1DConnector(
            ...
            apply_padding_mask=transformer_config.get("connector_apply_padding_mask", False),
        )
        return connector
...
class AudioEmbeddings1DConnectorConfigurator(ModelConfigurator[Embeddings1DConnector]):
    ...
    def from_config(cls: type[Embeddings1DConnector], config: dict) -> Embeddings1DConnector:
        ...
        connector = Embeddings1DConnector(
            ...
            double_precision_rope=double_precision_rope,
            apply_gated_attention=transformer_config.get("connector_apply_gated_attention", False),
            apply_padding_mask=transformer_config.get(
                "audio_connector_apply_padding_mask",
                transformer_config.get("connector_apply_padding_mask", False),
            ),
        )
        return connector
